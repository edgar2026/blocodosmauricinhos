...
import { paginate, getPaginationRange } from '../lib/pagination';
    ...
    try {
      const { error: updError } = await supabase
        ...
        .update({
          ...
          delivery_info_text: eventSettings.delivery_info_text,
    ...
    try {
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          food_kg: kg,
          food_type: foodType,
          notes: kg > 0 ? `Retirada de pulseira: ${foodType} (${kg}kg)` : `Retirada de pulseira: Sem doação de alimento`
        })
        .eq('id', participantId);
        ...
        {activeTab === 'overview' && (
          <>
            <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
              ...
              <div className="glass-card p-6 rounded-[2.5rem] shadow-2xl border-b-8 border-gray-400 group hover:-translate-y-2 transition-all relative overflow-hidden">
                <div className="absolute top-0 right-0 p-8 opacity-[0.03] group-hover:opacity-10 transition-opacity"><UserIcon size={100} /></div>
                <div className="flex items-center space-x-4 mb-4">
                  <div className="bg-gray-400 p-4 rounded-2xl shadow-lg"><UserIcon className="text-white" size={28} /></div>
                  <div>
                    <p className="text-[#0041B6] text-[10px] font-black uppercase tracking-[0.2em] opacity-60">Sem Doação</p>
                    <h3 className="text-[#0041B6] text-3xl font-black tracking-tighter">{loading ? '...' : totalSemDoacao.toLocaleString('pt-BR')}</h3>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="glass-card p-8 rounded-[2.5rem] shadow-xl border border-white/50 flex flex-col">
                <h4 className="text-[#0041B6] text-center font-black uppercase tracking-widest text-xs mb-10">Engajamento Solidário</h4>
                <div className="flex-1 min-h-[300px] relative">
                  {totalEntregues > 0 ? (
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-gray-300 py-10">
                      ...
                      <p className="text-[10px] font-black uppercase tracking-widest">Aguardando Entregas</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="glass-card p-8 rounded-[3rem] shadow-2xl border-4 border-[#FFD100]/30 animate-fade-in bg-white/40">
              <div className="flex flex-col gap-6 mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="relative group">
                    <div className="absolute left-6 top-1/2 -translate-y-1/2 text-[#0041B6]/30 group-focus-within:text-[#FFD100] transition-colors">
                      <MapPin size={18} />
                    </div>
                    <select
                      className="w-full bg-white/50 border-2 border-[#0041B6]/5 rounded-2xl py-4 pl-14 pr-10 focus:border-[#FFD100] focus:outline-none font-bold text-xs appearance-none cursor-pointer"
                      value={filterUnit}
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-[#0041B6] text-white">
                      ...
                      <th className="px-6 py-6 text-[10px] font-black uppercase tracking-widest text-center">Perfil</th>
                  ...
                  <tbody>
                    {loading ? (
                      <tr><td colSpan={6} className="text-center py-20"><Loader2 className="w-12 h-12 animate-spin inline-block text-[#FFD100]" /></td></tr>
                      ...
                      <tr key={p.id || index} className="group hover:bg-[#FFD100]/5 transition-colors border-b border-[#0041B6]/5 last:border-0">
                        ...
                        <td className="px-6 py-6 text-center"><span className="bg-[#1D71BC]/10 text-[#1D71BC] px-4 py-1 rounded-lg text-[10px] font-black uppercase">{p.user_type || 'Aluno(a)'}</span></td>
              ...
              {filteredParticipants.length > itemsPerPage && (
                <div className="mt-8 flex items-center justify-center gap-3">
                  <button
                    ...
                    className={`p-4 rounded-2xl transition-all shadow-sm ${currentPage === 1 ? 'bg-white/50 text-[#0041B6]/20 cursor-not-allowed' : 'bg-white text-[#0041B6] hover:bg-[#FFD100] hover:text-[#0041B6] active:scale-95 shadow-md'}`}
                    title="Página Anterior"
                  >
                    <ChevronLeft size={18} />
                  </button>

                  <div className="flex items-center gap-2 bg-white/50 p-1.5 rounded-[2rem] border-2 border-[#0041B6]/5">
                    {getPaginationPages().map(page => (
                      <button
                        key={page}
                        onClick={() => setCurrentPage(page)}
                        className={`w-12 h-12 rounded-[1.2rem] font-black text-xs transition-all ${currentPage === page ? 'bg-[#0041B6] text-white shadow-xl scale-110' : 'text-[#0041B6] hover:bg-[#0041B6]/10'}`}
                      >
                        {page}
                      </button>
                    ))}
                  ...
                  <button
                    ...
                    className={`p-4 rounded-2xl transition-all shadow-sm ${currentPage === totalPages ? 'bg-white/50 text-[#0041B6]/20 cursor-not-allowed' : 'bg-white text-[#0041B6] hover:bg-[#FFD100] hover:text-[#0041B6] active:scale-95 shadow-md'}`}
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      ...
                      <div className="space-y-4 md:col-span-3 pt-6 border-t border-[#0041B6]/5">
                        <label className="text-[10px] font-black uppercase text-[#0041B6]/50 ml-1 tracking-widest">Texto Informativo de Entrega</label>
                        <textarea
                          rows={2}
                          ...
                          value={eventSettings.delivery_info_text || ''}
                          onChange={e => setEventSettings({ ...eventSettings, delivery_info_text: e.target.value })}
                          placeholder="Ex: Em breve divulgaremos a data oficial para retirada das pulseiras..."
                        />
                      </div>
                      <div className="space-y-4 md:col-span-3 pt-6 border-t border-[#0041B6]/5">
            ...
            <div className="bg-white rounded-[3rem] w-full max-w-xl shadow-2xl overflow-hidden border-4 border-[#FFD100] animate-modal-enter">
              ...
              <div className="p-10 space-y-8">
                ...
                <div className="flex flex-col gap-4 pt-4">
                  <div className="flex gap-4">
                    <button onClick={() => setIsModalOpen(false)} className="flex-1 py-5 rounded-[2rem] font-black text-xs uppercase tracking-widest text-[#0041B6]/40 hover:bg-gray-100 transition-all font-fredoka">Cancelar</button>
                    <button onClick={() => handleConfirmDelivery(selectedParticipant.id!, 0, 'Sem Doação')} className="flex-1 bg-gray-100 text-[#0041B6]/60 py-5 rounded-[2rem] font-black text-xs uppercase tracking-widest hover:bg-gray-200 transition-all font-fredoka">Pular Doação</button>
                  </div>
                  <button disabled={!selectedFood || (selectedFood === 'Outros' && !customFood)} onClick={() => handleConfirmDelivery(selectedParticipant.id!, foodWeight, selectedFood === 'Outros' ? customFood : selectedFood)} className="w-full bg-[#2A9D8F] text-white py-5 rounded-[2rem] font-black text-xs uppercase tracking-widest shadow-xl hover:scale-105 transition-all">Confirmar com Doação ✨</button>
                </div>
              </div>
            </div>
          </div>
